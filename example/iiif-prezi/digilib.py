...
def i_make_manifest(app, issue_id):
...
def make_manifest(app, vol_id):
    ...
    for page_elem in orig_manifest['sequences'][0]['canvases']:
        ...
        canvas['images'][0]['resource']['@id'] = f'{image_url}/{page_id}.jpg/full/full/0/default.jpg'
        canvas['images'][0]['resource']['service']['@id'] = image_url
...
import pprint
class fake_app():
    def url_for(self,what, volume_id=None, doc_id=None):
        if volume_id: 
            return f"/{volume_id}/manifest"
        elif doc_id:
            return f"/{doc_id}/search"
    class config():
        def get(par1, par2):
            return par2
